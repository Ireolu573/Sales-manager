...
import { Trash2, Search, FileText, TrendingUp, ChevronDown, ChevronUp } from 'lucide-react'
...
// Extends Sale with optional staff email for admin view
...
function groupByDate(sales: SaleWithStaff[]): { label: string; sales: SaleWithStaff[] }[] {
  const groups: Record<string, SaleWithStaff[]> = {}
  for (const sale of sales) {
    const key = new Date(sale.sale_date).toDateString()
...
export default function SalesTable({ userId, tenantId, isAdmin, refreshKey, onDelete }: Props) {
  ...
  const [sales, setSales] = useState<SaleWithStaff[]>([])
  ...
  useEffect(() => {
    ...
    const fetchSales = async () => {
      ...
      if (isAdmin) {
        // Fetch all staff profiles for this tenant to map user_id -> email
        ...
        const enriched = (salesData as unknown as SaleWithStaff[]).map(s => ({
          ...s,
          staff_email: emailMap[s.user_id || ''] || undefined,
        }))
        setSales(enriched)
      } else {
        setSales(salesData as unknown as SaleWithStaff[])
      }
  ...
  const filtered = useMemo(() => {
    return sales.filter(s => {
      ...
      const matchesFilter =
        filter === 'All' || s.payment_method.toLowerCase() === filter.toLowerCase()
          ...
          <div className="bg-muted/60 border border-border/50 rounded-xl p-3">
            <div className="flex items-center gap-1.5 mb-1">
              <span className="text-xs text-muted-foreground font-medium">
                {filter === 'All' ? 'All time' : filter} total
              </span>
            </div>
            <div className="font-bold text-foreground text-base">
            ...
            <div key={group.label}>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-muted-foreground">
                  ₦{group.sales.reduce((s, r) => s + Number(r.total_amount), 0).toLocaleString()}
                </span>
              </div>
              ...
              <div className="space-y-2">
                {group.sales.map(sale => {
                  ...
                  return (
                    <Card
                      key={sale.id}
                      className={`border-border/50 shadow-sm overflow-hidden transition-all ${
                        isCredit && !isPaid ? 'border-warning/30 bg-warning/[0.03]' : ''
                      }`}
                    >
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-start gap-3 min-w-0">
                              ...
                              <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm shrink-0 ${
                                ...
                                payConfig.className.includes('warning') ? 'bg-warning/10' :
                                'bg-primary/10'
                              }`}>
                                {payConfig.icon}
                              </div>
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="text-xs text-muted-foreground mt-0.5">
                                  {sale.quantity} {sale.unit_label}
                                  {sale.customer_name && (
                                    <span> · {sale.customer_name}</span>
                                  )}
                                </div>
                                {/* Staff attribution for admins */}
                            ...
                            <div className="flex flex-col items-end shrink-0 gap-1">
                              ...
                              <div className="flex items-center gap-1.5">
                                <Badge variant="outline" className={`text-xs py-0 px-1.5 ${payConfig.className}`}>
                                  {payConfig.label}
                                </Badge>
                                {isExpanded
